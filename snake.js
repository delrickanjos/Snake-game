...
function Snake() {
    ...
    this.death = function() {
        ...
        for (let i = 0; i < this.tail.length; i++) {
            ...
            if (d < 1) { // Se sim, reiniciará o tamanho e o contador:
                this.total = 0;
                this.tail= [];
...
// Condição if que irá checar se alguma coisa foi comida:
        ...
        if (this.total >=1) { //Se algo já tiver sido comido, armazenar a localização atual para que a Cobra possa continuar se movendo:
...
// Função responsável por desenhar a cauda, e a Cobra em si, no Canvas. A função também define a cor da Cobra:
    this.show = function() {
        ...
        for (let i = 0; i < this.tail.length; i++) { // Loop responsável por desenhar a cauda inteira no Canvas, com base em sua posição no indice:
...
// Função para "comer" a Comida - Receberá a posição dela pelo vetor criado em pickLocation:
    this.eat = function(pos) {
        ...
        if (d < 1) { // O objetivo é confirmar se a Cobra alcançou a comida ou não através da distância entre a Cobra e a Comida. Se sim, somar um no contador:
            this.total++;
            return true;
        } else {
            return false;
